...
import LAL.AlgoritmoBubbleSort;
...
//#283044
    ...
    public void ordenarDocentesPorNombre() {
        ...
        List<Docente> listaDocentes = obtenerListaDocentesDesdeBD();

        // Aplicar el algoritmo de ordenamiento
        AlgoritmoBubbleSort.bubbleSort(listaDocentes);

        // Limpiar la tabla
        DefaultTableModel modelo = (DefaultTableModel) d.tablaDocente.getModel();
        modelo.setRowCount(0);

        // Iterar sobre la lista y agregar cada docente ordenado a la tabla
        int i = 0;
        for (Docente docente : listaDocentes) {
            i++;
            Object[] rowData = {
                // Puedes ajustar estos valores según las columnas de tu tabla
                i,
                docente.getCodigo(),
                docente.getDni(),
                docente.getCorreo(),
    ...
    private List<Docente> obtenerListaDocentesDesdeBD() {
        daoDocente dao = new daoDocente();
        return dao.obtenerListaDocentes();
    }
